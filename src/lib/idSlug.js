...
/**
 * ⚠️ جديد: ينظّف اسم ملف "حقيقي" (يظهر للمستخدم ويُستخدم كاسم فعلي على
 * GitHub) — بعكس transliterateToSlug أعلاه، هذي الدالة **لا تحوّل أي حرف
 * عربي للاتيني إطلاقاً**. الاسم يبقى بالضبط كما كتبه الآدمن (عربي/إنجليزي/
 * مختلط)، فقط تُزال المحارف الممنوعة فعلياً بمسارات GitHub/أنظمة الملفات
 * (`/ \ : * ? " < > |` والمحارف غير المرئية)، وتُقلَّص المسافات المتكررة،
 * وتُحذَف أي نقاط/مسافات زائدة بالبداية أو النهاية.
 *
 * القرار (بطلب مباشر من الإدارة، جلسة إصلاح تسمية الملفات): transliterateToSlug
 * يبقى كما هو *فقط* لمعرّفات النظام (id المادة، professorId) — لا علاقة لها
 * بهذي الدالة ولا تُستبدَل بها. اسم الملف الفعلي المرفوع (pdf/image) يجب أن
 * يطابق ما يراه المستخدم بالضبط، بلا أي تحويل صامت.
 */
